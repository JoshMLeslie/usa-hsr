...
fs.createReadStream('data-csv/cities-by-pop.csv')
	...
	.on('data', row => {
		...
		if (match) {
			...
			const city = groups[0].trim();
			const state = groups[1].trim();

			if (!cityToState[city]) {
				cityToState[city] = state;
			} else {
				cityToState.retry.push({city, state});
				console.warn(
					'duplicate city name: ' +
						city +
						', ' +
						state +
						'. Already assigned to state: ' +
						cityToState[city]
				);
			}
	...
	.on('end', () => {
		...
		writeResultsToJson(cityToState, 'city-to-state');
