...
export default class Rainbow {
	...
	constructor({
		...
		if (colors.some(c => c.startsWith('#'))) {
			colors = colors.map(c => (c.length === 7 ? c.substring(1) : c));
		}
		this.setColors(colors);
		this.setNumberRange(...range);
	}

	setColors(spectrum = this.colors) {
...
// Extended list of CSS color names taken from
